...
export const AutomatedAnalysisCardList: React.FC<AutomatedAnalysisCardListProps> = ({ evaluations }) => {
  ...
  const icon = React.useCallback((score: number): JSX.Element => {
    return score == AutomatedAnalysisScore.NA_SCORE ? (
      <span className={css('pf-m-grey', 'pf-v6-c-label__icon')}>
        <InfoCircleIcon />
      </span>
    ) : score < AutomatedAnalysisScore.ORANGE_SCORE_THRESHOLD ? (
      <span className={css('pf-m-green', 'pf-v6-c-label__icon')}>
        <CheckCircleIcon />
      </span>
    ) : score < AutomatedAnalysisScore.RED_SCORE_THRESHOLD ? (
      <span className={css('pf-m-orange', 'pf-v6-c-label__icon')}>
        <ExclamationTriangleIcon />
      </span>
    ) : (
      <span className={css('pf-m-red', 'pf-v6-c-label__icon')}>
        <ExclamationCircleIcon />
      </span>
    );
  }, []);
