...
const tableColumns: TableColumn[] = [
  {
    ...
    transform: (target: Target, _obj: ArchivesForTarget, t?: TFunction) => {
      return target.alias === target.connectUrl || !target.alias
        ? `${target.connectUrl}`
        : t
          ? t('AllTargetsHeapDumpsTable.TARGET_DISPLAY', {
              alias: target.alias,
...
export interface AllTargetsHeapDumpsTableProps {}

export const AllTargetsHeapDumpsTable: React.FC<AllTargetsHeapDumpsTableProps> = () => {
  ...
  const handleNotification = React.useCallback(
    (_: string, heapDump: HeapDump, delta: number) => {
      setArchivesForTargets((old) => {
        const matchingTargets = old.filter(({ target }) => {
          return target.jvmId === heapDump.jvmId;
        });
        for (const matchedTarget of matchingTargets) {
          ...
          const heapDumps = [...matchedTarget.heapDumps];
          if (delta === 1) {
            heapDumps.push(heapDump);
          } else {
            const heapDumpIdx = heapDumps.findIndex((t) => t.heapDumpId === heapDump.heapDumpId);
            heapDumps.splice(heapDumpIdx, 1);
          }

          old.splice(targetIdx, 1, { ...matchedTarget, archiveCount: matchedTarget.archiveCount + delta, heapDumps });
        }
            ...
            return v.data?.targetNodes
              ?.map((node) => {
                ...
                return {
                  target,
                  targetAsObs: of(target),
                  archiveCount: node?.target?.heapDumps?.aggregate?.count ?? 0,
                  heapDumps: (node?.target?.heapDumps?.data as HeapDump[]) ?? [],
                };
              })
              .filter((v) => !!v.target);
          }),
            ...
            setArchivesForTargets((old) => {
              return [
                ...old,
                {
                  target: target,
                  targetAsObs: of(target),
                  archiveCount: v.data.targetNodes[0]?.target?.heapDumps?.aggregate?.count ?? 0,
                  heapDumps: v.data.targetNodes[0]?.target?.heapDumps ?? [],
                },
              ];
            });
          }),
  ...
  if (isError) {
    view = (
      <>
        <ErrorView
          title={'Error retrieving Heap Dumps in All Targets View'}
        ...
        <ToolbarContent>
          <ToolbarGroup variant="filter-group">
            <ToolbarItem>
              <SearchInput
                style={{ minWidth: '30ch' }}
                placeholder={t('AllTargetsHeapDumpsTable.SEARCH_PLACEHOLDER')}
                value={searchText}
          ...
          <ToolbarItem alignSelf="center">
            <Checkbox
              ...
              label={t('AllTargetsHeapDumpsTable.HIDE_TARGET_WITH_ZERO_DUMPS')}
