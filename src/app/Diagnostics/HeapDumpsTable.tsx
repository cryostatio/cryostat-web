...
export const HeapDumpsTable: React.FC<HeapDumpsProps> = ({}) => {
  ...
  const handleDelete = React.useCallback(
    ...
    [addSubscription, refreshHeapDumps, context.api],
  );
  ...
  const handleWarningModalAccept = React.useCallback(() => {
    if (heapDumpToDelete) {
      ...
      notificationsContext.warning("Heap Dump Deletion Failure", "No heap dump to delete");
    }
  }, [handleDelete, heapDumpToDelete]);
  ...
  const handleHeapDump = React.useCallback(() => {
    addSubscription(
      context.api.runHeapDump(true).pipe(first()).subscribe({
        next: (jobId) => {
          addSubscription(
