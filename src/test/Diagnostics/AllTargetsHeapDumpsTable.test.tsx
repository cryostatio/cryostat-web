...
import { AllTargetsHeapDumpsTable } from '@app/Diagnostics/AllTargetsHeapDumpsTable';
import { HeapDump, NotificationMessage, Target } from '@app/Shared/Services/api.types';
...
const mockHeapDumpDeletedNotification = {
  message: {
    jvmId: mockTarget1.jvmId,
    heapDump: {
      jvmId: mockTarget1.jvmId,
      downloadUrl: 'foo',
      threadDumpId: mockHeapDump,
      lastModified: 0,
      size: 0,
    },
  },
...
jest.mock('@app/Diagnostics/HeapDumpsTable', () => {
  return {
    HeapDumpsTable: jest.fn((_) => {
      return <div>Heap Dumps Table</div>;
    }),
  };
});
...
jest
  ...
  .mockReturnValueOnce(of())
  .mockReturnValueOnce(of(mockHeapDumpDeletedNotification)) // decrements the count when an archived recording is deleted
  .mockReturnValueOnce(of());

jest.spyOn(window, 'open').mockReturnValue(null);

describe('<AllTargetsHeapDumpsTable />', () => {
  afterEach(cleanup);
  ...
  it('has the correct table elements', async () => {
    render({ routerConfigs: { routes: [{ path: '/heapdumps', element: <AllTargetsHeapDumpsTable /> }] } });
  ...
  it('hides targets with zero Heap Dumps', async () => {
    const { user } = render({
      routerConfigs: { routes: [{ path: '/heapdumps', element: <AllTargetsHeapDumpsTable /> }] },
    });
  ...
  it('correctly handles the search function', async () => {
    ...
    expect(screen.getByText('No Heap Dumps')).toBeInTheDocument();
  ...
  it('expands targets to show their <HeapDumpsTable />', async () => {
    const { user } = render({
      routerConfigs: { routes: [{ path: '/heapdumps', element: <AllTargetsHeapDumpsTable /> }] },
    });

    expect(screen.queryByText('Heap Dumps Table')).not.toBeInTheDocument();
    ...
    expect(within(expandedTable).getByText('Heap Dumps Table')).toBeTruthy();
  ...
  it('decrements the count when a Heap Dump is deleted', async () => {
    const { user } = render({
      routerConfigs: { routes: [{ path: '/heapdumps', element: <AllTargetsHeapDumpsTable /> }] },
    });
