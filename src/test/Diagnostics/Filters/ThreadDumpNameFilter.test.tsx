...
const mockAnotherThreadDump = { ...mockThreadDump, threadDumpId: 'anotherThreadDump' };
const mockThreadDumpList = [mockThreadDump, mockAnotherThreadDump];

const onNameInput = jest.fn((_nameInput) => {
  /**Do nothing. Used for checking renders */
});

describe('<ThreadDumpNameFilter />', () => {
  let emptyFilteredNames: string[];
  let filteredNames: string[];

  beforeEach(() => {
    emptyFilteredNames = [];
    filteredNames = [mockThreadDump.threadDumpId];
  });
        ...
        routes: [
          {
            ...
            element: (
              <ThreadDumpNameFilter
                threadDumps={mockThreadDumpList}
  ...
  it('should display name selections when text input is clicked', async () => {
    ...
    mockThreadDumpList.forEach((r) => {
      const option = within(selectMenu).getByText(r.threadDumpId);
  ...
  it('should not display selected names', async () => {
    ...
    const notToShowName = within(selectMenu).queryByText(mockThreadDump.threadDumpId);
  ...
  it('should select a name when a name option is clicked', async () => {
    ...
    await act(async () => {
      await user.click(screen.getByText('someThreadDump'));
    });

    expect(submitNameInput).toHaveBeenCalledTimes(1);
    expect(submitNameInput).toHaveBeenCalledWith('someThreadDump');
    expect(emptyFilteredNames).toStrictEqual(['someThreadDump']);
  });
});
