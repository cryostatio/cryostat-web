...
export const startMirage = ({ environment = 'development' } = {}) => {
  ...
  return createServer({
    ...
    routes() {
      ...
      // ── unified logging in-memory state ─────────────────────────────────────────────
      const unifiedLoggingState: Record<
        string,
        { enabled: boolean; logFilePath: string; what: string; decorators: string }
      > = {};
      const unifiedLogsList: Record<
        string,
      ...
      this.post('api/beta/diagnostics/targets/:targetId/unified-logging/pull', (_schema, request) => {
        ...
        websocket.send(
          JSON.stringify({
            meta: {
              category: 'UnifiedLogUploaded',
              type: { type: 'application', subType: 'json' },
            },
            message: { jvmId, unifiedLog: entry },
          }),
        );
        return new Response(200, {}, entry);
      });
          ...
          if (idx !== -1) {
            ...
            websocket.send(
              JSON.stringify({
                ...
                message: {
                  jvmId: removed.jvmId,
                  unifiedLog: { jvmId: removed.jvmId, logId, size: removed.size, lastModified: removed.lastModified },
                },
              }),
            );
          }
      ...
      this.post('api/beta/targets/:targetId/smart_triggers', (schema, request) => {
        ...
        const remoteId = Math.floor(Math.random() * 1000000);
        const trigger = schema.create(Resource.SMART_TRIGGER, {
          // id will generated by Mirage (i.e. increment integers)
          id: remoteId.toString(),
          expression: attrs.toString(),
          durationConstraint: 'foo-' + remoteId.toString(),
          triggerCondition: 'bar-' + remoteId.toString(),
          recordingTemplateName: 'baz-' + remoteId.toString(),
          state: 'WAITING_LOW',
          simple: true,
